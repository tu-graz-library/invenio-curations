...
from .diff import DiffBase, DiffProcessorBase
from .events import CurationCommentEventType

_generic_dict = dict[str, Any]
...
class CommentProcessor:
    ...
    def _cleanup_other_comments(
        ...
        for hit in events:
            if hit.get(
                "type"
            ) == "C" and not self._is_comment_hit_created_by_curations_comment_processor(
                hit
            ):
                continue
            result.append(hit)
    ...
    def _get_current_diffs(
        self, base_data: _generic_dict, new_data: _generic_dict
    ) -> list[tuple] | None:
    ...
    def _create_new_comment(
        self, request: _generic_dict, content: str, reference_draft: str | None
    ) -> None:
        ...
        try:
            current_events_service.create(  # type: ignore[attr-defined]
    ...
    def _update_existing_comment(
        self, new_data: str, crt_comment_event: _generic_dict
    ) -> None:
        ...
        try:
            current_events_service.update(  # type: ignore[attr-defined]
                self._identity,
    ...
    def _handle_critiqued_resubmit_status(
        self,
        request: _generic_dict,
        current_draft: _generic_dict,
        new_data: _generic_dict,
        msg: str,
        ...
        """
        last_event = list(
            current_events_service.search(self._identity, request["id"]).hits  # type: ignore[attr-defined]
        )[-1]
