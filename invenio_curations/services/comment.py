...
class CommentProcessor:
    ...
    def _create_new_comment(self, request, content, base_content_object):
        payload = {
            "payload": {"content": content, "base_content_object": base_content_object}
        }
    ...
    def _update_existing_comment(
        self, new_data, base_content_object, crt_comment_event
    ):
        if crt_comment_event is None:
            return self._comment_error

        payload = {
            "payload": {"content": new_data, "base_content_object": base_content_object}
        }
    ...
    def process_comment(self, request, errors):
        ...
        try:
            if request["status"] == "resubmitted":
                ...
                if (
                    last_request_log_event in ["resubmitted", "critiqued"]
                    and last_created_comment is None
                ):
                ...
                request["status"] == "critiqued"
                and len(self._diff_processor.get_diffs()) > 0
            ):
                ...
                if last_event.get("type") == "L":
                    ...
                    errors.append(
                        self._update_existing_comment(
                            self._diff_processor.compare(last_diff).to_html(
